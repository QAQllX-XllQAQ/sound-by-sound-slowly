...
Page({
  ...
  async getAIPronunciationAnalysis(content, score) {
    ...
    try {
      // 构建提示词
      const prompt = `请分析以下内容的发音并提供改进建议:
内容: "${content}"
评分: ${score}/100`;
      ...
      if (response.result && response.result.error) {
        ...
        wx.showToast({
          title: '分析失败: ' + response.result.error,
          icon: 'none',
          duration: 3000
        });
        return { tips: this.getDefaultFeedback(score) };
      ...
      wx.downloadFile({
        ...
        success: (res) => {
          if (res.statusCode === 200) {
            ...
            wx.hideLoading();
            wx.showToast({ title: '下载音频失败', icon: 'none' });
      ...
      success: res => {
        try {
          ...
          if (spectrogramData.length === 0) {
            ...
            this.setData({ showSpectrogram: false });
            wx.showToast({ title: '无法生成声谱图', icon: 'none' });
            return;
          }
  ...
  async runEvaluation() {
    ...
    try {
      ...
      // 使用默认反馈
      const score = Math.floor(Math.random() * 41) + 60;
      const scoreText = score >= 90 ? '优秀' : (score >= 80 ? '良好' : (score >= 70 ? '一般' : '需加强'));
